...
## Key Entry Points
- **Backend Server**: Run `uv run uvicorn main:app --port 8000 --reload` from project root
- **Frontend CLI**: Run `uv run python run_cli.py` from project root
- **Alternative CLI**: Direct entry via `uv run python src/client/spy_cli.py`
